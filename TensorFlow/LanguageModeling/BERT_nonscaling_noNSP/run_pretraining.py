...
# You may obtain a copy of the License atnfile ex
#
    ...
    if FLAGS.manual_fp16 or FLAGS.use_fp16:
      return tf.train.SessionRunArgs(
          fetches=['step_update:0', 'total_loss:0',
                   ...
                   'loss_scale:0']) # removed 'nsp_loss:0'
    else:
      return tf.train.SessionRunArgs(
          fetches=['step_update:0', 'total_loss:0',
                   'learning_rate:0', 'mlm_loss:0']) # removed  'nsp_loss:0'
  def after_run(self, run_context, run_values):
    self.elapsed_secs += time.time() - self.t0
    self.count += 1
    if FLAGS.manual_fp16 or FLAGS.use_fp16:
      global_step, total_loss, lr, mlm_loss, loss_scaler = run_values.results # removed  nsp_loss
    else:
      global_step, total_loss, lr, mlm_loss = run_values.results # removed nsp_loss
    ...
    if print_step == 1 or print_step % self.display_every == 0:
        ...
        if self.hvd_rank >= 0:
          if FLAGS.manual_fp16 or FLAGS.use_fp16:
            print('Rank = %2d :: Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f LR = %6.4e Loss scale = %6.4e' %
                  (self.hvd_rank, print_step, img_per_sec, mlm_loss, total_loss, lr, loss_scaler))
          else:
            print('Rank = %2d :: Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f LR = %6.4e' %
                  (self.hvd_rank, print_step, img_per_sec, mlm_loss, total_loss, lr))
        else:
          if FLAGS.manual_fp16 or FLAGS.use_fp16:
            print('Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f LR = %6.4e Loss scale = %6.4e' %
                  (print_step, img_per_sec, mlm_loss, total_loss, lr, loss_scaler))
          else:
            print('Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f LR = %6.4e' %
